...
st.set_page_config(page_title="Phonepe Pulse Data Visualization | By Pallav Ghoshal",
                   layout="wide",
...
conn = mysql.connector.connect(
    host='localhost', user='root', password='2v7lalqTcm9C4o3S', database='phone_pe_pulse')
...
with st.sidebar:
    selected = option_menu("Menu", ["Home", "Top Charts", "Explore Data", "About"],
                           icons=["house", "graph-up-arrow",
                                  "bar-chart-line", "exclamation-circle"],
    ...
    if Type == "Transactions":
        ...
        with col1:
            ...
            df = pd.DataFrame(mycursor.fetchall(), columns=[
                              'State', 'Transactions_Count', 'Total_Amount'])
        ...
        with col3:
            ...
            df = pd.DataFrame(mycursor.fetchall(), columns=[
                              'Pincode', 'Transactions_Count', 'Total_Amount'])
            fig = px.pie(df, values='Total_Amount',
                         names='Pincode',
                         title='Top 10',
                         color_discrete_sequence=px.colors.sequential.Agsunset,
...
# Top Charts - USERS
    if Type == "Users":
        ...
        with col2:
            ...
            df = pd.DataFrame(mycursor.fetchall(), columns=[
                              'District', 'Total_Users', 'Total_Appopens'])
        ...
        with col4:
            ...
            df = pd.DataFrame(mycursor.fetchall(), columns=[
                              'State', 'Total_Users', 'Total_Appopens'])
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            df1 = pd.DataFrame(mycursor.fetchall(), columns=[
                               'State', 'Total_Transactions', 'Total_amount'])
            df2 = pd.read_csv(r'D:/pulse/statenames.csv')
            df1.State = df2
...
# EXPLORE DATA - USERS
    if Type == "Users":
        ...
        st.markdown(
            "## :violet[Overall State Data - User App opening frequency]")
        mycursor.execute(
        ...
        df = pd.DataFrame(mycursor.fetchall(), columns=[
                          'State', 'year', 'quarter', 'District', 'Total_Users', 'Total_Appopens'])
